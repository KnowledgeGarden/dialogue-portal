  ...
  switch (action.type) {
    ...
    case SIGNUP_USER_SUCCESS:
      return state.merge({
        ...
        user: fromJS(action.payload.user, (key, value) => {
          if (key === 'role') return value.toSet();
          return value.toMap();
        })
      });
...
export function signupUser(dispatch, variables, redirect) {
  ...
  return new Promise(async function (resolve, reject) {
    const query = `
    mutation($name: String!, $handle: Handle!, $email: Email!, $password: Password!){
      payload: createUser(name: $name, handle: $handle, email: $email, password: $password)
       ${userWithAuthToken}
    }`;
    const {error, data} = await fetchGraphQL({query, variables});
    if (error) {
